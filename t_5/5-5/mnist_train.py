...
def train(mnist):
    ...
    y = inference(x, avg_class=None, reuse=False, lamada=LAMADA)  # 未使用滑动平均
    ...
    # 计算使用了滑动平均的网络前向传播结果，滑动平均不是改变变量本身的值，而是维护一个影子变量来记录其滑动平均值
    # 因此当需要使用这个滑动平均值的时候，需要明确调用average函数
    average_y = inference(x, variable_averages, reuse=True, lamada=None)  # 使用滑动平均
    ...
    # 计算在当前batch里所有损失的交叉熵平均值，并加入损失集合
    ...
    # 又需要更新每一个参数的滑动平均值。为了一次完成多种操作，tensorflow提供了两种机制。
