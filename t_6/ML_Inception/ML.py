...
def create_image_lists(testing_percentage, validation_percentage):
    ...
    # 获取当前目录下所有的子目录
    # 遍历给定的根目录下的所有文件夹，每个文件夹返回一个三元组(该文件夹路径，该文件夹下的文件夹名字，该文件夹下的文件名称（不包含文件夹）)
    ...
    for sub_dir in sub_dirs:
        ...
        for extension in extensions:
            ...
            # 返回所有匹配的文件路径列表，并加入列表
            file_list.extend(glob.glob(file_glob))  # glob模块的主要方法就是glob,该方法返回所有匹配的文件路径列表（list）
        if not file_list:
            continue
        # 通过目录名获取类别名称
...
def main():
    ...
    n_classes = len(image_lists.keys())  # 5 因为flower_photos文件夹下有五个子文件夹
        ...
        for i in range(STEPS):
            ...
            if i % 100 == 0 or i + 1 == STEPS:
                ...
                print('Step %d: Validation accuracy on random sampled %d examples = %.1f%%' %
                      (i, BATCH, validation_accuracy * 100.00))
